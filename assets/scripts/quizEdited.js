...
const questions = [
  ...
  },
  {
    ...
    options: [
      ...
      "I would be overwhelmed and would not know where to start with such an amount",
      "I would be relieved that my financial future is now secure",
      "I would be very happy that I can now make my wishes and those of my family come true",
...
const func = ()=>{
  const dodge = localStorage.dodger;
const scroog = localStorage.scrooge;
const abbote = localStorage.abbot;
const  splurge = localStorage.splurger
const scoreArr = [dodge, scroog, abbote, splurge]
  const r = scoreArr.every(i=> Number(i)>=0)
  if(r) return  localStorage.setItem('visited', true )
}
...
$(".btn.btnMain").addEventListener(
  ...
  e => {
    ...
    $(".quiz-form").reset();
    if(localStorage.visited) return location.assign("https://vested.ng/html/result.html");  
    // if(localStorage.visited) return location.assign("http://127.0.0.1:5503/html/result.html");  
